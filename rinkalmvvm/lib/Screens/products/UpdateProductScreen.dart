...
class UpdateProductScreen extends StatefulWidget {
  var updateid = "";
  UpdateProductScreen({required this.updateid});
...
class _UpdateProductScreenState extends State<UpdateProductScreen> {
  ...
  getsinglerecord() async {
    ...
    // var params = {"pid": widget.updateid};
    // var response = await http.post(url, body: params);
    // if (response.statusCode == 200) {
    //   var json = jsonDecode(response.body.toString());
    //
    // }
    ApiHandler.getcall("http://picsyapps.com/studentapi/getSingleProduct.php")
        .then((json) {
      setState(() {
        if (json["status"] == "true") {
              ...
              Center(
                child: ElevatedButton(
                    onPressed: () async {
                      ...
                      Uri url = Uri.parse(
                          "http://picsyapps.com/studentapi/updateProductNormal.php");
                      var params = {
                        "pname": name,
                        "qty": qty,
                        "price": price,
